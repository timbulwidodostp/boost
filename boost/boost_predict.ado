...
*! version 1.2.1  Oct 4, 2020,  Matthias Schonlau
	...
	if "`L'" == "1" {
		...
		qui replace `predictname'=. if !`touse'
		//note: this does not test for distribution=bernoulli, because the info is not in ereturn
		if ("`class'"!="")  qui replace `predictname' = round(`predictname')  // round probabilities
	}
	else {
		...
		foreach i of numlist 1/`L' {
			...
			qui replace `predictname'`i' =. if !`touse'
		}
...
Revision history
Version date 
1.2.1   Oct 4, 2020	  Added "qui" in front of several "replace"
